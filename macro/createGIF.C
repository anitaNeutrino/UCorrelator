...
void createGIF() 
{
   ...
   gSystem->Unlink("../drivers/a4all/movie.gif"); // delete old file
   TCanvas* c = new TCanvas();
   TFile *f = new TFile("../drivers/a4all/140_max_300000_sinsub_10_3_ad_2.root");
   if (f == 0) {
      printf("Error: cannot open root file.\n");
      return;
   }
   TTree *anita3 = (TTree*) f->Get("anita4");
   ...
   for (int ientry = 0; ientry < num_entries; ientry+=step) 
   {
         ...
         anita3->Draw("-1*summary->peak[0][0].theta:(summary->peak[0][0].phi- pat.heading +360)%360>>h2","(summary->coherent_filtered[0][0].peakPower>12)","colz",step, ientry);
         // hist = new TH2D("h2",histName,360,0.0,360.0,300,-90.0,90.0);
