...
// must point, and be bigger in the right polarity 
TCut isMC = "abs(FFTtools::wrap(peak[][].phi-mc.phi,360,0)) < 5 && abs(FFTtools::wrap(peak[][].theta-mc.theta,360,0)) < 3 && ( ( mc.wf[0].peakHilbert > mc.wf[1].peakHilbert  &&  Iteration$ < 5) || ( mc.wf[1].peakHilbert > mc.wf[0].peakHilbert && Iteration$ >= 5))"; 
TCut anyMC = "Sum$((abs(FFTtools::wrap(peak[][].phi-mc.phi,360,0)) < 5 && abs(FFTtools::wrap(peak[][].theta-mc.theta,360,0)) < 3)  > 0)"; 
